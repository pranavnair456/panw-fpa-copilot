...
The "full attribution" detail tables are relabeled and number-formatted **only in the dashboard** (`app/dashboard.py`: `LINE_RENAME`, `money_table`, `variance_view`) — `src/variance.py` keeps its precise rounded data and original line keys, so the saved JSON, the Stage 5 narrative, and the tests are untouched. For the non-technical viewer this means: each table title carries the *baseline* and a one-line plain-English caption (e.g. "Did we beat the targets we told investors?"), so the rows only need the *subject* — `Midpoint / Low / High`, `Product` / `Subscription & support`. In "vs our forecast" the two rows are spelled out as `Organic revenue (like-for-like)` (the real beat) and `Total reported revenue (includes acquisition)` (the gap between them *is* the acquisition).

### One money-formatting standard (display layer)
Every dollar/percentage in the app flows through **one shared formatter** (`src/fmt.py`: `fmt_money`, `fmt_pct`, `money_cell`, `col_scale`, `fmt_eps`), imported by both the dashboard and the text generators (`variance.py`, `summary.py`, `chat.py`, `anomaly.py`) — no ad-hoc per-tab f-strings. Rules: a standalone figure **≥ $1B shows in billions** (`$3.00B`, `$18.40B`), otherwise millions (`$594M`); **dollar negatives use finance parentheses** (`$(50)M`); **percentages are one decimal with a sign** (`+2.0%`, `-1.9%` — parens are dollars-only). Tables/charts auto-scale **per column/axis** to $B or $M by typical magnitude, with the unit stated in the header/axis (`Actual ($B)`, `Revenue ($B)`); a column is never mixed. This is display only — underlying values, logic, and reconciliations are unchanged. Crucially, the **no-hallucination verifier was extended** (`src/verify.py`) to parse the new formats — `$X.XXB` normalizes back to $M, and `$(50)M` to a negative — *before* matching, so the headline switch to `$3.00B` and parenthesized negatives can't break verification (the $B rounding error, ≤ $5M, sits well inside the existing max($1M, 1.5%) tolerance). Tests prove a correct brief in the new format still verifies and a wrong one is still caught.
